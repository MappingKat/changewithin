...
# -------------------------------------------
if 'AREA_GEOJSON' in heroku config:
    config.set('area', 'geojson', heroku config:get['AREA_GEOJSON'])

if 'MAILGUN_DOMAIN' in heroku config:
    config.set('mailgun', 'domain', heroku config:get['MAILGUN_DOMAIN'])

if 'MAILGUN_API_KEY' in heroku config:
    config.set('mailgun', 'api_key', heroku config:get['MAILGUN_API_KEY'])

if 'EMAIL_RECIPIENTS' in heroku config:
    config.set('email', 'recipients', heroku config:get['EMAIL_RECIPIENT1'])
...
f_out = open('report_%s.html' % now.strftime("%m-%d-%y"), 'w')
